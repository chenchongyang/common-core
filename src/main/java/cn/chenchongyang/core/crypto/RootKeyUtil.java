...
public class RootKeyUtil {
    ...
    private static final String ROOT_PART3 =
        "";

    private static final String ROOT_SALT = "154D3B72D66CAF9222DFA60B88B7ACC5";

    public static byte[] getRootKey() {
        byte[] c1 = EncodeUtil.hexToByte(ROOT_PART1);
        byte[] c2 = EncodeUtil.hexToByte(ROOT_PART2);
        byte[] c3 = EncodeUtil.hexToByte(ROOT_PART3);

        int minLength = getMinLength();
        char[] password = new char[minLength];
        for (int i = 0; i < minLength; i++) {
            password[i] = (char) (c1[i] ^ c2[i] ^ c3[i]);
        }
        return PBKDF2Util.encryptPBKDF2WithSHA256(password, EncodeUtil.hexToByte(ROOT_SALT));
    }

    public static byte[] getWorkKey(byte[] rootKey) {
        try {
    ...
    private static int getMinLength() {
        return Stream.of(ROOT_PART1.length(), ROOT_PART2.length(), ROOT_PART3.length())
